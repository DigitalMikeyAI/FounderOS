...
(function () {
  ...
  const result = document.getElementById("operating-agenda-result"); const rangeNode = document.getElementById("operating-agenda-range"); const checkedAt = document.getElementById("operating-agenda-checked-at"); const refresh = document.getElementById("operating-agenda-refresh");
  if (!result || !rangeNode || !checkedAt || !refresh) return;
  ...
  const week = (evaluation) => { const current = ymd(evaluation); if (!current) return null; const weekday = utcCalendar(current).getUTCDay(); const monday = addDays(current, -((weekday + 6) % 7)); const nextMonday = addDays(monday, 7); const from = midnight(monday); const to = midnight(nextMonday); return from && to ? { from, to, monday, sunday: addDays(nextMonday, -1) } : null; };
  const rangeLabel = (range) => { const month = (date) => new Intl.DateTimeFormat("en-US", { timeZone: UTC, month: "long" }).format(utcCalendar(date, 12)); const sameYear = range.monday.year === range.sunday.year; const sameMonth = sameYear && range.monday.month === range.sunday.month; return `${sameMonth ? `${month(range.monday)} ${range.monday.day}` : `${month(range.monday)} ${range.monday.day}, ${range.monday.year}`}–${sameMonth ? `${range.sunday.day}, ${range.sunday.year}` : `${month(range.sunday)} ${range.sunday.day}, ${range.sunday.year}`} · Eastern time`; };
  ...
  const render = (projection, request) => {
    ...
    if (projection.status === "unavailable") next.appendChild(element("p", "operating-briefing-state", "This week's agenda couldn't be checked. Try refreshing."));
    else {
      ...
      for (const item of projection.items) { const begins = item.kind === "commitment-deadline" ? item.occursAt : item.opensAt; const group = item.kind === "routine-occurrence" && item.opensAt < request.from ? groupFor("continuing", "Continuing into this week") : (() => { const date = ymd(begins); return date ? groupFor(`${date.year}-${date.month}-${date.day}`, dateFormatter.format(new Date(begins))) : null; })(); if (!group) return false; group.items.push(item); }
      ...
      if (projection.items.length === 0) next.appendChild(element("p", "operating-briefing-state", projection.status === "partial" ? "No items were returned from the information available." : "No recorded deadlines or recurring windows fall in this week."));
    }
    clear(); result.appendChild(next); return true;
  };
  const unavailable = () => { clear(); result.appendChild(element("p", "operating-briefing-state", "This week's agenda couldn't be checked. Try refreshing.")); };
  const evaluate = () => {
    const evaluation = new Date().toISOString(); const selected = week(evaluation); if (!selected) { unavailable(); checkedAt.textContent = ""; rangeNode.textContent = ""; return; }
  ...
  refresh.disabled = true; refresh.addEventListener("click", evaluate);
  const start = () => { if (window.__operatingAgendaStarted) return; window.__operatingAgendaStarted = true; refresh.disabled = false; evaluate(); };
